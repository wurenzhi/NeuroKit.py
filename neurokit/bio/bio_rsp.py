...
#   ============
    ...
    power, freq = mne.time_frequency.psd_array_multitaper(rsp, sfreq=sampling_rate, fmin=0,
                                                          fmax=0.5, adaptive=False,
                                                          normalization='length')
    processed_rsp["RSP"] = {}
    processed_rsp["RSP"]["power_bands"] = {}
    for band in freq_bands:
        processed_rsp["RSP"]["power_bands"][band] = power_in_band(power, freq, freq_bands[band])
...
#   ============
    ...
    processed_rsp["RSP"]["statistics"]['SDFD']=np.std(np.diff(data_rspr))
    processed_rsp["RSP"]["statistics"]['MSD']=np.mean(np.diff(data_rspr, 2))
    processed_rsp["RSP"]["statistics"]['SDSD']=np.std(np.diff(data_rspr, 2))
    processed_rsp["RSP"]["statistics"]['SDBA']=np.std(data_waveform)
    processed_rsp["RSP"]["statistics"]['MAXRSP']=np.max(data_waveform)
    processed_rsp["RSP"]["statistics"]['MINRSP']=np.min(data_waveform)
    processed_rsp["RSP"]["statistics"]['DMMRSP']= processed_rsp["RSP"]["statistics"]['MAXRSP'] - processed_rsp["RSP"]["statistics"]['MINRSP']
    processed_rsp["RSP"]["statistics"]['Skewness']=scipy.stats.skew(data_rspr)
    processed_rsp["RSP"]["statistics"]['Kurtosis']=scipy.stats.kurtosis(data_rspr)
